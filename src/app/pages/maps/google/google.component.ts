...
export class GoogleComponent implements OnInit {
  ...
  _initPanorama() {
    if (isPlatformBrowser(this.platformId)) {
      this.mapsAPILoader.load().then(() => {
        const center = { lat: 21.176518, lng: 105.889894 };
        // tslint:disable-next-line: no-string-literal
        const map = new window['google'].maps.Map(this.streetviewMap.nativeElement, { center, zoom: 100, scrollwheel: false });
        // tslint:disable-next-line: no-string-literal
  ...
  private _fetchData() {
    this.markers = [
      { latitude: 20.972253, longitude: 105.758245 }
    ];
  }
